param(
  [string[]] $Cities    = @("Fukuoka","CNX","Singapore"),
...
function Run-One {
  ...
  $log_dijkstra = Join-Path $LogDir ("{0}_{1}ps_dijkstra_{2}.log" -f $name, $Density, $stamp)

  $scriptPath = Join-Path $Root "FinalnoTLS.py"
  if (-not (Test-Path $scriptPath)) {
    throw "Cannot find FinalnoTLS.py at: $scriptPath"
  }

  Write-Host "[$name $Density ps] Running FinalnoTLS.py with cfg '$cfgFile'"
  Stop-LeftoverSumo
